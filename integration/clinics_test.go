...
var _ = Describe("Clinics", func() {
	...
	Describe("GetWithCounts", func() {
		It("returns patients per site", func() {
			...
			clinicWithCounts, err := clinicsSvc.GetWithCounts(ctx, clinic.Id.Hex())
			Expect(err).To(Succeed())
			Expect(len(clinicWithCounts.Sites)).To(Equal(2))
			var theSite *sites.Site
			for _, site := range clinicWithCounts.Sites {
		...
		It("returns patients per tag", func() {
			...
			clinicWithCounts, err := clinicsSvc.GetWithCounts(ctx, clinic.Id.Hex())
			Expect(err).To(Succeed())
			Expect(len(clinicWithCounts.PatientTags)).To(Equal(3))
			var theTag *clinics.PatientTag
			for _, tag := range clinicWithCounts.PatientTags {
