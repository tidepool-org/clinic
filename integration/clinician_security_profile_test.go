...
var _ = Describe("Clinician Security Profile Integration Test", Ordered, func() {
	...
	Describe("Update Clinician Security Profile", func() {
		...
		It("Applies an MFA event", func() {
			...
			var profile client.ClinicianSecurityProfileV1
			Expect(json.NewDecoder(res.Body).Decode(&profile)).To(Succeed())
	...
	Describe("Disable MFA", func() {
		...
		It("Clears mfaEnabledTime when mfaEnabled is set to false", func() {
			rec := httptest.NewRecorder()
			req := prepareRequest(http.MethodPatch, endpoint, "./test/securityprofile_fixtures/04_mfa_disabled_event.json")
			asServer(req)
			...
			var profile client.ClinicianSecurityProfileV1
			Expect(json.NewDecoder(rec.Result().Body).Decode(&profile)).To(Succeed())
	...
	Describe("Field-scoped partial updates", func() {
		...
		It("Does not clobber mfaEnabled when an unrelated event arrives", func() {
			...
			Expect(patch("./test/securityprofile_fixtures/01_mfa_event.json").StatusCode).To(Equal(http.StatusOK))
