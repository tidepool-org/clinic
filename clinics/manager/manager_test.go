	...
	Context("Sites", func() {
		Describe("CreateSite", func() {
			...
			It("allows duplicate site names between different clinics", func() {
				...
				clinic2, _ := th.createTestClinic(GinkgoTB())
				Expect(mngr.CreateSite(ctx, th.Clinic.Id.Hex(), th.Site.Name)).To(Succeed())
		...
		// While ListSite is a function on the clinics.Service interface (not
		...
		// much easier.
		Describe("ListSite", func() {
			It("works", func() {
				ctx, _, th := newCreateSiteTestHelper(GinkgoTB())

				clinicId := th.Clinic.Id.Hex()

				//Expect(mngr.ListSites(ctx, clinicId)).To(Succeed())
...
func newCreateSiteTestHelper(t testing.TB) (context.Context, manager.Manager, *createSiteTestHelper) {
	...
	testClinicInput := test.RandomClinic()
...
func (c *createSiteTestHelper) createTestClinic(t testing.TB) (*clinics.Clinic, *clinicians.Clinician) {
	testClinic := test.RandomClinic()
	testClinician := cliniciansTest.RandomClinician()
