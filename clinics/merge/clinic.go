...
func (m *ClinicMergePlanner) listAllPatients(ctx context.Context, clinic clinics.Clinic) ([]patients.Patient, error) {
	...
	// are around 471.2 B in size. 100 MB / 512 B == 195312.5, but to be extra conservative
	// against future increases in this size, and because clinic users are more likely to
	// have summaries than not, we'll set to the limit to 100000 patients.
	//
	// For the above analysis I used this query:
	//   db.patients.aggregate([{$limit:200000},{$addFields:{__tmp_cgm__:"$summary.cgmStats.dates.lastUpdatedDate",__tmp_bgm__:"$summary.bgmStats.dates.lastUpdatedDate"}},{$unset:"summary"},{$addFields:{"summary.bgmStats.dates.lastUpdatedDate":"$__tmp_bgm__","summary.cgmStats.dates.lastUpdatedDate":"__tmp_cgm__"}},{$unset:["__tmp_cgm__","__tmp_bgm__"]}, {$replaceWith: { size: {$bsonSize: '$$ROOT'}} },{ $group: { _id: null, count: { $sum: 1 }, total: { $sum: '$size' }, avg: { $avg: '$size' } } }, { $unset: '_id' } ])
	limit := 100000
