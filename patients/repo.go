...
func PatientsToTideResult(patientsList []*Patient, period string) []TideResultPatient {
	...
	for _, patient := range patientsList {
		...
		categoryResult = append(categoryResult, TideResultPatient{
			...
			AverageGlucoseMmol:         patient.Summary.CGM.Periods[period].AverageGlucoseMmol,
			GlucoseManagementIndicator: patient.Summary.CGM.Periods[period].GlucoseManagementIndicator,
			TimeCGMUseMinutes:          patient.Summary.CGM.Periods[period].TimeCGMUseMinutes,
			TimeCGMUsePercent:          patient.Summary.CGM.Periods[period].TimeCGMUsePercent,
			TimeInHighPercent:          patient.Summary.CGM.Periods[period].TimeInHighPercent,
			TimeInLowPercent:           patient.Summary.CGM.Periods[period].TimeInLowPercent,
			TimeInTargetPercent:        patient.Summary.CGM.Periods[period].TimeInTargetPercent,
			TimeInTargetPercentDelta:   patient.Summary.CGM.Periods[period].TimeInTargetPercentDelta,
			TimeInVeryHighPercent:      patient.Summary.CGM.Periods[period].TimeInVeryHighPercent,
			TimeInVeryLowPercent:       patient.Summary.CGM.Periods[period].TimeInVeryLowPercent,
			TimeInAnyLowPercent:        patient.Summary.CGM.Periods[period].TimeInAnyLowPercent,
			TimeInAnyHighPercent:       patient.Summary.CGM.Periods[period].TimeInAnyHighPercent,
			LastData:                   patient.Summary.CGM.Dates.LastData,
		})
	}
...
func (r *repository) TideReport(ctx context.Context, clinicId string, params TideReportParams) (*Tide, error) {
	...
	for _, category := range categories {
		...
		tide.Results[category.Heading] = PatientsToTideResult(patientsList, *params.Period)
	...
	if limit > 0 {
		...
		tide.Results["meetingTargets"] = PatientsToTideResult(patientsList, *params.Period)
	}

	{
		...
		tide.Results["noData"] = PatientsToTideResult(patientsList, *params.Period)
