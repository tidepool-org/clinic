...
func (r *repository) List(ctx context.Context, filter *Filter, pagination store.Pagination, sorts []*store.Sort) (*ListResult, error) {
	...
	if filter.ExcludeSummaryExceptFieldsInMergeReports {
		pipeline = excludeSummaryExceptFieldsInMergeReports(pipeline)
	}
...
func excludeSummaryExceptFieldsInMergeReports(in []bson.M) []bson.M {
	out := in
	out = append(out, bson.M{"$addFields": bson.M{
		"__tmp_cgm__": "$summary.cgmStats.dates.lastUpdatedDate",
		"__tmp_bgm__": "$summary.bgmStats.dates.lastUpdatedDate",
	}})
	out = append(out, bson.M{"$unset": "summary"})
	out = append(out, bson.M{"$addFields": bson.M{
		"summary.cgmStats.dates.lastUpdatedDate": "$__tmp_cgm__",
		"summary.bgmStats.dates.lastUpdatedDate": "$__tmp_bgm__",
	}})
