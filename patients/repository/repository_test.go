...
var _ = Describe("Patients Repository", func() {
	...
	BeforeEach(func() {
		...
		repo, err = patientsRepository.NewRepository(cfg, database, zap.NewNop().Sugar(), nil, lifecycle)
...
func newTestRepo(t FullGinkgoTInterface, dataCounts patientDataCounts, withoutData int) (
	...
	logger := testLogger()
	repo, err := patientsRepository.NewRepository(cfg, database, logger, nil, lifecycle)
	Expect(err).ToNot(HaveOccurred())
	Expect(repo).ToNot(BeNil())
	lifecycle.RequireStart()
	ctx := context.Background()
