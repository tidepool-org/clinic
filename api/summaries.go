...
func (h *Handler) ListCGMSummaries(ec echo.Context, clinicId ClinicId, params ListPatientSummariesParams) error {
	...
	t := summary.GetTypeString[summary.CGMPeriod]()
...
func (h *Handler) ListBGMSummaries(ec echo.Context, clinicId ClinicId, params ListPatientSummariesParams) error {
	ctx := ec.Request().Context()
	page := pagination(params.Offset, params.Limit)
	t := summary.GetTypeString[summary.BGMPeriod]()
	...
	list, err := h.bgmsummaries.List(ctx, &filter, page, sorts)
	if err != nil {
		return err
	}
...
func (h *Handler) UpdatePatientBGMSummary(ec echo.Context) error {
	ctx := ec.Request().Context()
	var dto *PatientSummary
	if ec.Request().ContentLength != 0 {
		dto = &PatientSummary{}
	...
	for _, period := range periods {
		s, err := NewSummary[summary.BGMPeriod](dto, period)
		err = h.bgmsummaries.CreateOrUpdate(ctx, s)
		if err != nil {
			return err
		}
	}
